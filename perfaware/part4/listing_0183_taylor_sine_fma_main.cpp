...
inline f64 TaylorSineBackward(u32 MaxPower, f64 X)
{
    f64 Result = 0;
    ...
    for(u32 InvPower = 1; InvPower <= MaxPower; InvPower += 2)
    {
        ...
        f64 XPow = X;
        for(u32 XMul = 3; XMul <= Power; XMul += 2)
        {
            XPow *= X2;
        }
...
inline f64 TaylorSineBackwardFMA(u32 MaxPower, f64 X)
{
    f64 Result = 0;
    ...
    for(u32 InvPower = 1; InvPower <= MaxPower; InvPower += 2)
    {
        ...
        Result = fma(XPow, TaylorSineCoefficient(Power), Result);
    }
    
    return Result;
}

inline f64 TaylorSineHornerFMA(u32 MaxPower, f64 X)
{
    f64 Result = 0;
    ...
    {
        u32 Power = MaxPower - (InvPower - 1);
        Result = fma(Result, X2, TaylorSineCoefficient(Power));
    }
    Result *= X;
...
int main(void)
{
    ...
    for(u32 Power = 1; Power <= 31; Power += 2)
    {
        while(PrecisionTest(&Tester, 0, Pi64/2))
        {
            ...
            TestResult(&Tester, RefOutput, TaylorSineBackward(Power, Tester.InputValue), "TaylorSineBackward%u", Power);
            TestResult(&Tester, RefOutput, TaylorSineHorner(Power, Tester.InputValue), "TaylorSineHorner%u", Power);
            TestResult(&Tester, RefOutput, TaylorSineBackwardFMA(Power, Tester.InputValue), "TaylorSineBackwardFMA%u", Power);
            TestResult(&Tester, RefOutput, TaylorSineHornerFMA(Power, Tester.InputValue), "TaylorSineHornerFMA%u", Power);
        }
    }
