...
static f64 SimplifiedHaversineJ(haversine_setup Setup)
{
    // NOTE(casey): Remove unnecessary sign handling
    ...
    for(u64 PairIndex = 0; PairIndex < PairCount; ++PairIndex)
    {
        ...
        f64 NegRadC = -0.01745329251994329577;
        f64 NegPi = -3.14159265358979323846;
        f64 HalfRadC = 0.00872664625997164788;
        f64 HalfPi = 1.57079632679489661923;
        f64 Deg180 = 180.0;
        
        f64 AbsDLat = fabs(lat2 - lat1);
        f64 AbsDLon = fabs(lon2 - lon1);
        f64 AbsLat1 = fabs(lat1);
        f64 AbsLat2 = fabs(lat2);
        f64 ALC0 = (AbsDLat < Deg180) ? 0 : NegPi;
        f64 ALC3 = (AbsDLon < Deg180) ? 0 : NegPi;
        
        f64 S1 = SineCoreWithPrefix(NegRadC, AbsLat1, HalfPi);
        f64 S2 = SineCoreWithPrefix(NegRadC, AbsLat2, HalfPi);
        f64 S0 = SineCoreWithPrefix(HalfRadC, AbsDLat, ALC0);
        f64 S3 = SineCoreWithPrefix(HalfRadC, AbsDLon, ALC3);
        
        f64 a = fma(S0, S0, S1*S2*S3*S3);
        
        b32 NeedsTransform = (0.5 < a);
        f64 RangeA = NeedsTransform ? (1.0 - a) : a;
        f64 R = ArcsineCoreFromSquared(RangeA);
        f64 RangeR = NeedsTransform ? (HalfPi - R) : R;
...
static test_function TestFunctions[] =
{
    ...
    {"SimplifiedHaversineJ", SimplifiedHaversineJ},
};
